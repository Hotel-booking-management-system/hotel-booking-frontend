...
fetch(apiUrl)
    ...
    .then(data => {
        ...
        for (const roomName in dataByName) {
            ...
            btn.classList.add("w-44", "rounded-t-md", "border-t-[3px]", "border-gray-600", "text-black", "font-bold");
            btn.addEventListener('click', () => {
                room.innerHTML = '';
                const getdata = dataByName[roomName] ?? dataByName[roomName];
                if (currentBtn) {
                    currentBtn.style.backgroundColor = "white";
                    currentBtn.classList.remove('text-gray-50');
                }
...
function createRoomCard(data) {
    ...
    img.classList.add('rounded-lg', 'h-48', 'w-full', 'mb-4');
    ...
    if (data.available) {
        ...
        h2.classList.add('text-green-500', 'py-1', 'float-right', 'text-right', 'font-semibold')
    } else {
        h2.textContent = `${month[dateObject.getMonth()]} ${getDayWithSuffix(dateObject.getDate())} ,${getHourWithAMPM(dateObject.getDate())}`;
        h2.classList.add('text-red-400', 'py-1', 'float-right', 'font-semibold')
    }
    ...
    h4.classList.add('py-1', 'w-full')


    const h5 = document.createElement('h5');
    h5.textContent = `Accessories - ${data.accessories}`;
    h5.classList.add('pt-1', 'max-sm:py-1.5')

    let div = document.createElement('div')
    div.classList.add('px-10', 'py-6')
